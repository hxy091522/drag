...
Jh.fn=function(a){
	return a={
		...
		_createLis:function(b,c){$.each(b,function(b,d){c.append(a._createLi(b,d))})},
		_createA:function(a){
			return $("<a href='javascript:void(0);' rel='"+a+"'>"+a+"</a>")
		},
		_createLi:function(a,c){return $("<li/>").append("<a href='#' rel='"+a+"'>"+c+"</a>").append("<span class='ok'></span>")},
		...
		_bindSave:function(a){
			a.click(function(){
				...
				f="";$("."+Jh.Config.layCls+" a").each(function(){
					$(this).hasClass("active")&&(f=$(this).attr("rel"))
				});
				"1:1:1"==f&&(f="\u9ed8\u8ba4");
				$.fallr("show",{content:"<p>left:["+a+"]</p><p>center:["+b+"]</p><p>right["+d+"]</p><p>\u5f53\u524d\u5e03\u5c40:"+f+"</p>",position:"center"})
			})
		},
		_bindEvent:function(){
			a._moduleLiClick();
		...
		_ToLayout:function(a){
			...
			$.each(Jh.Layout.layoutCss,function(c,j){a==j&&(d=c)});
			$.each(e,function(a,b){var g=$(b),h=c[d].split(/\s+/);
			switch(a){case "left":a=0;break;case "center":a=1;break;case "right":a=2}"wnone"==h[a]&&(f=g.sortable("toArray"),$.each(f,function(a,b){$("#"+Jh.Layout.location.left).append($("#"+b))}),g.empty());
...
Jh.Portal=function(a){
	var b="<div id='"+Jh.Layout.location.left+"' class='"+Jh.Config._groupWrapperClass+" w250'/>",c="<div id='"+Jh.Layout.location.center+"' class='"+Jh.Config._groupWrapperClass+" w250'/>",e="<div id='"+Jh.Layout.location.right+"' class='"+Jh.Config._groupWrapperClass+" w250'/>",d="<div id='{key}' class='"+Jh.Config._groupItemClass+"'/>",f="<div class='"+Jh.Config._groupItemHead+"'><h3>{name}</h3></div>",i="<div class='"+Jh.Config._groupItemContent+"'/>";
	return a={
		init:function(b){
			a._create();
			a._bindData(b);
		...
		_createA:function(a,b,c){
			a=$("<a href='javascript:void(0);' class='"+a+"' title='"+b+"'/>");
			c||a.hide();
			return a
		},
		_eventMin:function(){
			$("."+Jh.Config.min).on("click",function(){
				var a=$(this),b=a.parent().parent().parent();
				b.find("."+Jh.Config._groupItemContent).hide();
				b.find("."+Jh.Config.max).show();a.hide()
			})
		},
		_eventMax:function(){
			$("."+Jh.Config.max).on("click",function(){
				var a=$(this),b=a.parent().parent().parent();
		...
		_eventRemove:function(){
			$("."+Jh.Config.close).on("click",function(){
		...
		_eventRefresh:function(){
			$("."+Jh.Config.refresh).on("click",function(){
